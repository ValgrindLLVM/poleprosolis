...
pub struct GameMaps<UI: ui::Context> {
    pub current_map: GameMap,
    pub farm: Vec<BlockData<UI>>,
}
pub enum GameMap {
    Farm,
}

impl<UI: ui::Context> GameMaps<UI> {
    ...
    pub fn get_current(&self) -> &Vec<BlockData<UI>> {
        match self.current_map {
            GameMap::Farm => &self.farm,
        }
    }
    /// Get current map (blocks on it)
    pub fn get_current_mut(&mut self) -> &mut Vec<BlockData<UI>> {
        match self.current_map {
    ...
    pub fn find_at(&self, pos: Point) -> Option<&BlockData<UI>> {
        self.get_current().iter().find(|p| p.state.pos == pos)
    }
    /// Find block by position
    pub fn find_at_mut(&mut self, pos: Point) -> Option<&mut BlockData<UI>> {
    ...
    pub fn do_random_tick(&mut self, game_handle: &mut GameHandle<UI>) -> Result<(), UI::Error> {
        let mut other_updates = Vec::new();
        let mut states: Vec<&BlockState> = Vec::new();
        for (block, state) in self
            .get_current_mut()
            .iter_mut()
            .filter_map(|BlockData { state, block }| block.as_mut().map(|b| (b, state)))
        {
...
pub struct BlockData<UI: ui::Context> {
    /// Block generic state
    pub state: BlockState,
    /// Local block state and behavior
    pub block: Option<Box<dyn Block<UI>>>,
}
/// Type of collision
...
impl<UI: ui::Context> BlockData<UI> {
    /// Creates new block
    pub fn new(
        pos: Point,
        collision: CollisionTy,
        ty: BlockTy,
        block: Option<Box<dyn Block<UI>>>,
    ) -> Self {
        Self {
